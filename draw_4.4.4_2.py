...
if __name__ == "__main__":
    ...
    file_paths = [
        ...
        "/sharenvme/usershome/cyl/test/prefill_data/重排实验：prefill_log_nocache_sort_re_maxsize=1_q.txt"
    ]
    
    # 【核心修复】：为两条曲线设置标准的学术名称
    label_map = {
        "重排实验：prefill_log_nocache_sort_maxsize=2_q.txt": "w/ Expert Reorder (Ours)",
        "重排实验：prefill_log_nocache_sort_re_maxsize=1_q.txt":"w/o Expert Reorder (Baseline)", 
    }
    ...
    if has_valid_data:
        ...
        fig.tight_layout()
        plt.savefig('prefill_latency_smoothed_4.4.4_2.pdf', format='pdf', bbox_inches='tight')
        plt.savefig('prefill_latency_smoothed_4.4.4_2.png', bbox_inches='tight')
        print("平滑绘图成功！已保存为 prefill_latency_smoothed_4.4.4_2.pdf")
    else:
        print("无法绘图：未读取到有效数据")