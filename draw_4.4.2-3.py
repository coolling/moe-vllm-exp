...
if __name__ == "__main__":
    ...
    label_map = {
        "prefill_init_m.txt": "Full Resident",
        "prefill_ondemand_m.txt": "On-Demand",
        "prefill_layerwise_m.txt": "Seq Prefetch",
        "prefill_cache_m.txt": "Static Cache",
        "prefill_moevllm_cache_2_m.txt": "EffiMoE (Ours)",
        "prefill_init_q.txt": "Full Resident",
        "prefill_ondemand_q.txt": "On-Demand",
        "prefill_layerwise_q.txt": "Seq Prefetch",
        "prefill_cache_q.txt": "Static Cache",
        "prefill_moevllm_cache_15_q.txt": "EffiMoE (Ours)"
    }
    ...
    if has_valid_data:
        ...
        fig.tight_layout()
        plt.savefig('prefill_latency_smoothed_4.4.2-3-q.pdf', format='pdf', bbox_inches='tight')
        plt.savefig('prefill_latency_smoothed.png', bbox_inches='tight')
...
def smooth_curve(x, y, num_points=500, sigma=2):
    """
    使用三次样条插值 + 高斯滤波 生成极致平滑曲线
    :param sigma: 平滑度控制参数，值越大曲线越平滑（抹平震荡），默认设为 12
    """
...
if __name__ == "__main__":
    ...
    file_paths = [
        ...
        "/sharenvme/usershome/cyl/test/prefill_data/prefill_ondemand_m.txt",
        "/sharenvme/usershome/cyl/test/prefill_data/prefill_layerwise_m.txt",
    ...
    if has_valid_data:
        ...
        fig.tight_layout()
        plt.savefig('prefill_latency_smoothed_4.4.2-3-m.pdf', format='pdf', bbox_inches='tight')
        plt.savefig('prefill_latency_smoothed_4.4.2-3-m.png', bbox_inches='tight')
        print("平滑绘图成功！已保存为 prefill_latency_smoothed.pdf 和 prefill_latency_smoothed.png")
    else:
        print("无法绘图：未读取到有效数据")